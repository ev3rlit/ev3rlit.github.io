...
import { useThemeBridge } from './useThemeBridge';
    ...
    const handleMount: OnMount = (editor, monaco) => {
        ...
        editor.addCommand(monaco.KeyCode.Tab, () => {
            ...
            if (model && position) {
                ...
                // Check if it's a list item (unordered or ordered)
                const listMatch = lineContent.match(/^\s*([-*+]|\d+\.)\s/);

                if (listMatch) {
                    // Only indent line if cursor is at the beginning or right after the marker
                    // match[0].length gives the length of indentation + marker + space
                    // position.column is 1-based, so if cursor is at end of marker, column = length + 1
                    if (position.column <= listMatch[0].length + 1) {
