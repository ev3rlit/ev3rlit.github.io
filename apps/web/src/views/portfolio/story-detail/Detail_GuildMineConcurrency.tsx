...
export const Detail_GuildMineConcurrency = () => (
    <SwissProjectDetail
        ...
        architectureDescription={[
            ...
            "### 측정 없는 최적화의 한계",
            "",
            "멱등성 키와 분산 락을 전 커맨드에 적용했지만, **실제로 동시성 충돌이 얼마나 발생했는지 측정하지 못했습니다.** 충돌 빈도를 먼저 측정하고, 문제가 확인된 부분에만 복잡한 해법을 적용했다면 더 효율적이었을 것입니다.",
            "",
            "### 이번 프로젝트에서 배운 것",
            "",
            "분산 서버 환경에서 공유 자원(길드 광산, 이송 수레)의 동시성을 제어하는 방법, Redis 분산 락과 낙관적 동시성 제어의 조합, asynq를 활용한 비동기 태스크 예약까지 — 실시간 멀티플레이어 시스템을 처음부터 설계하고 운영하면서 가장 많이 성장한 프로젝트였습니다.",
            "",
            "### 다음에 개선하고 싶은 것",
            "",
            "이번에 가장 아쉬웠던 점은 **관측성(Observability)의 부재**입니다. 충돌 빈도, 분산 락 대기 시간, 낙관적 동시성 제어 재시도 횟수, 이벤트 처리 지연 같은 지표를 수집했다면 최적화 포인트를 정량적으로 판단할 수 있었을 것입니다. 다음 프로젝트에서는 **측정 → 판단 → 적용** 순서를 지키는 관측성 체계를 먼저 구축하고 싶습니다."
        ].join("\n")}
        mainTasks={[
            {
